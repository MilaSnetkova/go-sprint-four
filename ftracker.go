...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	//находим среднюю скорость в км/ч
	meanSpeedKmh := meanSpeed(action, duration)
	//рассчитываем потраченные калории по формуле:
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// Находим среднюю скорость при ходьбе в м/с
	meanSpeedMps := meanSpeed(action, duration) * kmhInMsec
	//рассчитываем потраченные калории при ходьбе по формуле:
	//((0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах)* 0.029 * ВесСпортсменаВКг) * ВремяТренировкиВЧасах * minInH)
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMps, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// Находим среднюю скорость при плавании
	meanSpeedKmh := swimmingMeanSpeed(lengthPool, countPool, duration)
	// находим  потраченные калории при плавании по формуле
